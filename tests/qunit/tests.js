...
QUnit.test('testAliasSegmentMethod', function(assert) {
  var obj = new BreakpointX();
  obj.addDevice('iphone', 414);
  obj.aliasSegment(500, 'medium');
  assert.deepEqual(obj.getSegment('medium'), obj.getSegment('iphone'));
});

QUnit.test('testMediaMediaMinAndMediaMaxKeysAreCorrect', function(assert) {
  ...
  assert.strictEqual(segment['mediaMin'], '(min-width:414px)');
  assert.strictEqual(segment['mediaMax'], '(max-width:767px)');
...
QUnit.test('Assert AddByScreenWidth works as expected', function(assert) {
  ...
  assert.deepEqual(
    ['min_0', 'min_321', 'min_769', 'min_1337'],
    obj.segmentNames
  );
